...
如果在dev分支不commit直接回到主分支，会发现自己在dev分支写的代码master分支也出现了，ev分支写的代码影响了主分支，所以需要先commit再切换分支
...
git merge的缺点：当合并两个没有上下游关系的分支时，git会自动生成一个merge commit，记录此次的merge，记录merge操作没有什么问题，问题是如果遇到特殊情况需要反复merge的时候，就会**导致commit的提交记录非常混乱**
...
![image-20220314094353062](https://gitee.com/zqylzcwcxy/picture-bed/raw/master/img/image-20220314094353062.png)

这张图非常经典，是很多场景下的常态。C1是线上的版本，在C1的代码上线了之后发现了一个bug，于是checkout了一个叫做bugFix的分支。与此同时还有新的功能在开发，新的功能提交到了master之后形成了节点C2。这个时候在bugFix分支当然可以merge master这没有什么问题，但是也可以rebase master，rebase之后整棵git树会变成这样：

![image-20220314094421378](https://gitee.com/zqylzcwcxy/picture-bed/raw/master/img/image-20220314094421378.png)

这个结果就好像是**先到了C2然后checkout出了bugFix分支，然后在bugFix分支上将之前写过的代码重新写了一遍**，这样的操作就是`变基`，当rebase了之后再提交合并请求的合并记录里面会非常干净，没有多余merge的信息。对于多人协同开发的场景非常有帮助，具体命令如下
...
![image-20220314111938343](https://gitee.com/zqylzcwcxy/picture-bed/raw/master/img/image-20220314111938343.png)
