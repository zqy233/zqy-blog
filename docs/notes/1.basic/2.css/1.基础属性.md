b4650bede2e72e14ff55a8f06bbbead8
...
```bash
w15px + tab -> width:15px;
h15px + tab -> height:15px;
m15px + tab -> margin:15px;
p15px + tab -> padding:15px;
lh15px + tab -> line-height:15px;
tac + tab -> text-align:center;
bg + tab -> background:
bgc + tab -> background-color:
brs + tab -> border-radius:
c + tab -> color:
...
flex盒子下，子元素即使设置了固定宽度，如果flex的宽度小于各子元素宽度之和，则会自动等比例缩小各子元素，从而导致子元素的宽度小于固定宽度
...
:root 表示一个HTML文档的根（顶级元素），所以它也被称为根元素
