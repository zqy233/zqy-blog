...
闰年-366天，平年365天，闰年2月多了个29日

:::

闰年需要的结果：

- 今年是闰年，今天2月28，去年也是2月28
- 今年是闰年，今天2月29，去年是3月1，因为去年没有2月29
- 今年是闰年，今天3月1，去年是3月1
...
function isLeap31(d) {
  ...
  const less31 = new Date(year, month, day).getTime() < new Date(year, 3, 1).getTime()
  if (isLeap && !less31||less31 && lastYearIsLeap) {
    return -366
  }
  return -365
}
...
function lastYearDay() {
  return getTime(new Date(new Date(d).setDate(new Date(d).getDate() + isLeap31(d))))
}
