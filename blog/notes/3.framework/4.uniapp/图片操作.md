...
> 下方代码涉及图片上传和下载，数据库表中数据新增、查询、删除

## uniapp上传图片流程

1. 调用`uni.chooseImage`选择图片，可单选也可多选(小程序最多九张)，成功的话会返回图片的临时路径数组
...
6. 所以删除图片，不仅要删除数据库表中的数据，还要删除云存储中的文件

## 选择图片

微信小程序最多选择九张，超过九张，只有前九张会在图片的临时路径数组中返回
